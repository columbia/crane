...
analysis_tools="--worker1=drcov --worker2=helgrind"                                   # for executing analysis tools (e.g., helgrind)

if [ $proxy -eq 1 ]
then
    if [ $leader_elect -eq 1 ]
    then
        client_cmd="${msmr_root_client}/apps/apache/install/bin/ab -n 128 -c 8 http://${primary_ip}:9000/test.php"
    else
        client_cmd="${msmr_root_client}/apps/apache/install/bin/ab -n 128 -c 8 http://${primary_ip}:9000/test.php"
    fi
else
    client_cmd="${msmr_root_client}/apps/apache/install/bin/ab -n 128 -c 8 http://${primary_ip}:7000/test.php"
fi

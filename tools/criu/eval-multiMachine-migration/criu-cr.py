...
def criu_dump(pid, identity, after_dump_status):
	...
	feedback = Popen(cmd, env=cur_env, shell=True, executable='/bin/bash', stdout=PIPE, stderr=PIPE)
...
def criu_cr():
	...
	# 'mg-server' is the name of mongoose server process
	# 'server.out' is the name of libevent_paxos process
	pid_s = get_pid('mg-server')
	...
	if pid_s > 0 and pid_c > 0:
		...
		wake_check(pid_s, 'mg-server')
		wake_check(pid_c, 'server.out')
	elif pid_s == 0 :
		print "< the mg-server process does not exit >"
	elif pid_c == 0:
		print "< the server.out process does not exit >"

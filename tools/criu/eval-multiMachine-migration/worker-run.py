...
#MSMR_ROOT = ''
#XTERN_ROOT = ''
MSMR_ROOT = '/home/bluecloudmatrix/m-smr'
XTERN_ROOT = '/home/bluecloudmatrix/m-smr/xtern_with_paxos'
...
def execute_proxy(args):
    ...
    if not os.path.isfile('nodes.local.cfg'):
        ...
        tcmd = 'cp /home/bluecloudmatrix/m-smr/libevent_paxos/target/nodes.local.cfg .'
    ...
    if not os.path.isfile('local.options'):
        ...
        tcmd = 'cp /home/bluecloudmatrix/m-smr/xtern_with_paxos/default.options ~/local.options'
        subprocess.Popen(tcmd, env=cur_env, shell=True, stdout=subprocess.PIPE)
...
def execute_servers(args):
    ...
    if args.node_id == 2:
        cmd = "tmux send -t checkpoint:0 '/home/bluecloudmatrix/m-smr/apps/mongoose/mg-server -t 2' ENTER"
    else:
        cmd = args.scmd
    ...
    p = subprocess.Popen(cmd, env=cur_env, shell=True, executable='/bin/bash', stdout=subprocess.PIPE)
